/**
 * Resize full AI social images (v5 batch) to exact export sizes.
 * Input: print8-full-v5-{service}-{square|portrait|story}.png in CURSOR_ASSETS
 */
...
const OUT = path.resolve(
  process.env.PRINT8_SERVICES_OUT ??
    path.join(home, 'Desktop', 'print8-social-services-v5'),
);
...
async function main() {
  ...
  for (const job of JOBS) {
    const rawName = `print8-full-v5-${job.service}-${job.suffix}.png`;
    ...
    const out = path.join(
      OUT,
      outDir,
      `print8-${job.service}-${job.suffix}-v5.jpg`,
    );
  ...
  console.log(`Done — ${done} full-AI v5 exports → ${OUT}`);
}
