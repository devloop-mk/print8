...
const assets = path.resolve(
  process.env.WOMEN_MOCKUP_ASSETS ?? resolveCursorAssetsDir(),
);
const outDir = path.join(process.cwd(), 'public', 't-shirts', 'women');
const slugs = ['bela', 'crna'];
...
async function processMockup(srcPath, destPath, normalizeBackground) {
  let pipeline = normalizeBackground
    ? await normalizeLightBackgroundToWhite(srcPath)
    : sharp(srcPath);
...
for (const slug of slugs) {
  for (const side of ['front', 'back']) {
    const src = path.join(assets, `women-${slug}-${side}.png`);
    ...
    await processMockup(src, dest, slug === 'crna');
