/**
 * Resize full AI social images to exact export sizes (no overlay compositing).
 * Input: print8-full-v3-{service}-{square|portrait|story}.png in CURSOR_ASSETS
 */
...
const OUT = path.resolve(
  process.env.PRINT8_SERVICES_OUT ??
    path.join(home, 'Desktop', 'print8-social-services-v3'),
);
...
async function main() {
  ...
  for (const job of JOBS) {
    const raw = path.join(RAW, `print8-full-v3-${job.service}-${job.suffix}.png`);
    try {
      await fs.access(raw);
    } catch {
    ...
    const out = path.join(
      OUT,
      outDir,
      `print8-${job.service}-${job.suffix}-v3.jpg`,
    );
  ...
  console.log(`Done — ${done} full-AI exports → ${OUT}`);
}
