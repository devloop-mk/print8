...
async function main() {
  ...
  if (!databaseUrl) {
    console.error(
      [
        ...
        '  File: supabase/migrations/add-managed-svg-templates.sql',
      ].join('\n'),
    );
    process.exit(1);
  }
  ...
  const sqlPath = path.join(
    process.cwd(),
    'supabase/migrations/add-managed-svg-templates.sql',
  );
  ...
  try {
    ...
    console.log('managed_svg_templates table is ready.');
  } finally {
