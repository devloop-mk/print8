...
export function DesignsGallery() {
  ...
  function updateCategory(next: DesignCategory | 'all') {
    setCategory(next);
    setSubfilter('all');
    router.replace(buildDesignsHref(next, 'all'), { scroll: false });
  }

  function updateSubfilter(next: DesignSubfilterId | 'all') {
    setSubfilter(next);
    router.replace(buildDesignsHref(category, next), { scroll: false });
  }
  ...
  const filtered = filterDesignsBySubfilter(
    filteredByCategory,
    subfilter,
